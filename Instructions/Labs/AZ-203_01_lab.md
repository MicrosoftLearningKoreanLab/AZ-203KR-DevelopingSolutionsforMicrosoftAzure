...
11. Use the following script to connect to the VM that you created earlier in this lab by using the SSH tool and the IP address stored in the Bash shell variable *ipAdwdress*:
...
5.  Use the **dotnet new console --output . --name ipcheck** command to create a new .NET Core console application in the current directory.
...
3.  **Enable** the **Admin User** and save your changes to the settings.
...
3.  Select/right-click the **latest** tag for the **ipcheck** container image to deploy a new **Azure container instance** with the following settings:
